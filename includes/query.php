    ...
    function QUERY_POPULATE_RECORDS_TABLE($var1)
    {
        ...
        if ($result->num_rows > 0)
        {
            while ($row = $result->fetch_assoc())
            {
                ...
                <td class='text-center'><a data-id2='".$row['transaction_no']."' name='released' class='released' style=$releasedStyle>Released</a><a data-id2='".$row['transaction_no']."' name='delivered' class='delivered' style=$deliveredStyle>Delivered</a><a id='editModal' name='editModal' class='editModal' data-id2='".$row['transaction_no']."' style='cursor:pointer;'>Update</a><a href='invoice.php?date=".$row['date']."&name=".$row['fullname']."&items=".$row['item_name']."&qty=".$row['quantity']."&presc=".$row['prescription']."&totalPrc=".$row['total_price']."&rcv=".$row['amount_receive']."&bal=".$row['balance']."' target='_blank' id='invoice' name='invoice'>Invoice</a></td> 
